...
StewardHQ is a Resource Governance Gateway: a stewardship runtime that sits between AI agents and legacy business APIs. Agents never own resources (invoices, orders, money) — they are granted **temporary, bounded stewardship**: borrows, leases, and linear capabilities, enforced at runtime. Think "Rust's borrow checker for business resources," implemented on the BEAM.

**Core thesis: agents propose; the steward disposes.** The resources are protected _from_ the agents.

The authoritative design document is `docs/spec.md` (StewardHQ Technical Specification v2.2). Read it before non-trivial work. If code and spec disagree, flag it — don't silently pick one.

## Stack

- **Elixir** (latest stable), OTP supervision throughout
- **Ash Framework** — shadow resources, actions, preparations/changes
- **Spark DSL** — the `Steward.Resource` extension where safety laws are declared
- **Reactor** — saga execution (`run` / `compensate` / `undo`)
- Postgres via AshPostgres for durable saga state
- Namespace: `Steward.*` (product name StewardHQ appears only in docs/marketing)

## Architecture map

```
Steward.Registry        — active borrows + capability tokens (incl. moves)
Steward.ResourceServer  — GenServer per resource: local lock arbitration, borrow queue
Steward.Resource        — Spark DSL extension; compiles laws → Resource Safety Graph
Steward.PlanValidator   — static analysis of plan graphs before execution
Steward.Changes.EnforceFencing — Ash change: version check + fencing token on writes
Steward.LeaseProvider   — behaviour (v2); v1 is a hardcoded GenServer implementation
```

Build order is fixed: **Lock Manager first (Phase 1), then leases/fencing, then DSL, then sagas, then capability movement + MCP facade.** Don't build ahead of the current phase; don't abstract behaviours before Phase 1–4 code proves the seams.

## Invariants — never violate these

1. **No unwitnessed access.** Every Ash action on a stewarded resource requires a `borrow_token` in changeset context. Missing token → `{:error, :unborrowed_access}`. There is no bypass path, including from our own internal code.
2. **No unfenced write.** Every mutating call to a backend carries a fencing token and version condition. A write without them must be impossible to express, not merely discouraged.
3. **Capabilities are linear.** Moved capabilities are consumed at the sender (`{:error, :capability_moved}` on reuse). Success consumes `consumption: :linear` capabilities. Disposition on failure depends on error type: `:stale_resource` → survives (resync + retry); `:invalid_transition` → dropped.
4. **No capability outlives its plan.** Sub-agent tasks are supervised and plan-scoped. Process death must release borrows automatically (`Process.monitor`, never trust cooperative cleanup).
5. **Locks release only after consistency.** If a backend call succeeded but reconciliation failed, run the repair loop (re-fetch until consistent) _before_ releasing the borrow.
6. **Irreversible steps go last.** The PlanValidator rejects plans that schedule an `:irreversible` step before a fallible one — `{:error, :irreversible_before_fallible, step_id}`.
7. **Errors are structured tuples, never bare booleans/exceptions across module boundaries.** The taxonomy in spec §4.4 is the contract agents recover from; adding a new error shape requires updating the spec first.

## Conventions

- **Errors:** `{:error, reason_atom, context}` — reason atoms from the spec taxonomy only. Raise only for programmer errors (invalid config, impossible states), never for domain outcomes.
- **State machines:** transitions only through the DSL-declared graph. Direct `Ecto.Changeset` state writes are forbidden.
- **Shadow state is a cache, never truth.** Every shadow record carries `sync_status :: :synced | {:stale, age_ms} | :conflicted` and a version token. Code that reads shadow state for a mutation decision must check freshness policy.
- **GenServers:** one `ResourceServer` per resource id, registered via `Registry`. Keep them small — arbitration only, no business logic. Timeouts on every call; no infinite `receive`.
- **Naming:** stewardship vocabulary in public APIs — `grant_stewardship/3`, `release/2`, `borrow/3` — not "lock"/"unlock".
- **Tests:** every invariant above needs a test that tries to violate it and asserts the structured error. Concurrency paths get property/race tests (spawn N agents contending for one resource; assert exactly one exclusive holder at any time). Crash paths get tests that kill the holder process and assert auto-release.
- **No mocks for the borrow checker.** Use real GenServers in tests; mock only the legacy-API boundary (define it behind a behaviour from day one — this is the one seam v1 does abstract).

## Things NOT to do

- Don't add auth/RBAC features — stewardship is not authorization; it composes with authz, it doesn't replace it.
- Don't implement webhook/CDC ingestion, wound-wait deadlock arbitration, or pluggable `LeaseProvider` backends yet (deferred; see spec §8–9).
- Don't "improve" borrow semantics with reentrant locks or upgradeable read→write borrows without a spec change — those alter the safety model.
- Don't catch-and-continue on reconciliation failures. Escalate to the repair loop or dead-letter queue.
- Don't write to `docs/spec.md` casually — spec changes are deliberate, versioned edits.

## Current phase

**Phase 1 — Core Kernel.** Deliverables:

- `Steward.Registry` (borrow + capability token tracking)
- `Steward.ResourceServer` with `acquire(resource_id, mode: :exclusive | :shared)` / `release(resource_id)`, borrow queueing, `Process.monitor`-based auto-release
- `borrow(resource_id, mode, fun)` — function-scoped borrow with guaranteed release on any exit
- Test suite covering: shared/shared coexistence, shared/exclusive blocking, exclusive/exclusive queueing, crash auto-release, double-release safety

Definition of done for any phase: all invariant-violation tests pass, `mix credo --strict` and `mix dialyzer` clean, spec cross-references in moduledocs (`@moduledoc` cites the spec section it implements).
...
These positional arguments are _very important_ for certain cases, because there may be values you do not want the form to be able to set. For example, when updating a user's settings, maybe the action takes a `user_id`, but the form is on a page for a specific user's id and so this should therefore not be editable in the form. Use positional arguments for this.
