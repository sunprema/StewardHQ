...
See [`docs/examples.md`](docs/examples.md) for a few concrete scenarios these mechanisms are meant for — paying an invoice, cancelling an order, a supervisor agent delegating approval to a sub-agent. [`docs/cookbook.md`](docs/cookbook.md) makes the first two of those actually runnable, dev-only, against two dummy backend APIs that simulate the real quirks (version conflicts, on-demand failures) worth guarding against.
