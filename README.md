...
- **Borrow checking.** Every action on a protected resource needs a `borrow_token` — and not just any term in the right place: the token is checked against the resource server that issued it, so it only works for the process that acquired it, only while the borrow is held, and only on the resource it was taken on. No borrow, no access, including from your own application code. `Steward.borrow({Invoice, id}, :exclusive, fn token -> ... end)` runs your function with that token and releases the borrow when it returns, crashes, or the process holding it dies.
...
See [`docs/examples.md`](docs/examples.md) for a few concrete scenarios these mechanisms are meant for — paying an invoice, cancelling an order, a supervisor agent delegating approval to a sub-agent. [`docs/cookbook.md`](docs/cookbook.md) makes the first two of those actually runnable, dev-only, against dummy backend APIs that simulate the real quirks (version conflicts, on-demand failures) worth guarding against — including one that makes real HTTP calls against a Stripe-shaped mock server, no Stripe account required.
